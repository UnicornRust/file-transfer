use crate::{display, help::{BUFFER_LENGTH, DEFAULT_PORT}};
use std::{
...
pub fn send(args: &[&str]) {
    // 获取的 Ip 和端口号
    let socket_addr: SocketAddrV4 = match args[0] {
    ...
    if path.is_file() {
        ...
        display::display_file_info(path, &total_length, length);
    } else {
        //
        ...
        paths.iter().for_each(|p| {
            ...
            display::display_file_info(p, &total_length, transferred_length);
        })
    }
}
